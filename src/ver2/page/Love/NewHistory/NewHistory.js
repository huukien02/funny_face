...
export default function NewHistory() {
  ...
  useEffect(() => {
    axios
      .get(
        `https://databaseswap.mangasocial.online/lovehistory/comment/${stt_su_kien}?id_toan_bo_su_kien=${id}`
      )
      .then((response) => {
  ...
  const fetchDataUser = async () => {
    ...
    try {
      const response = await axios.get(
        `https://databaseswap.mangasocial.online/lovehistory/${id}`
      );
              ...
              <div className=" flex justify-center">
                <ul className="flex flex-col gap-y-8 w-full ">
                  ...
                  {dataUser?.id_user === id_user ? (
                    <li
                      className={`cursor-pointer flex  text-center justify-center items-center hover:bg-[#d34229] rounded-3xl lg:py-10 lg:px-36 py-6 px-2 ${
                        isActive === 0 ? "bg-[#d32929] text-white" : ""
                      }`}
                      onClick={() => redirect(0)}
                    >
                      Add Event
