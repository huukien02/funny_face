    ...
    if (!id) {
      try {
        ...
        const response = await fetch(
          `https://databaseswap.mangasocial.online/lovehistory/page/${count}?id_user=${idUser}`
        );
        if (!response.ok) {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(
          `https://databaseswap.mangasocial.online/lovehistory/page/${id}?id_user=${idUser}`
        );
        if (!response.ok) {
          throw new Error("Failed to fetch data");
        }
        ...
        <div>
          {props.data.map((array, index) => (
            <div
              ...
              key={index}
              className={`w-full h-[230px] mx-3 lg:h-[380px] mb-4 border-8 border-pink-300  bg-white rounded-[36px]`}
            >
        ...
        <div className="component-events">
          {currentResults.map((array, index) => (
            <div
              ...
              key={index}
              // className={`w-[600px] h-[230px] mx-3 lg:h-[380px] mb-4 border-8 border-pink-300  bg-white rounded-[36px]`}
